...
- 已按旧静态页展示方式纠偏素材篮：素材篮内容等同当前勾选素材，通过右侧抽屉展示缩略图、格式/大小、有效期和底部批量操作。
- 已实现素材组基础管理：当前素材组可编辑名称/父级，可软删除素材组并将组内素材移入回收站。
- 已实现标签编辑与合并：标签可编辑名称/描述/状态，合并会转移关联并软删除源标签。
- 已按旧静态页展示方式纠偏标签管理页：保留业务标签/AI 标签 tab、搜索/排序、表格列、顶部新增/合并入口和新增/编辑/合并弹框。
...
- 迁移功能必须以 `legacy-static/` 的页面结构、弹框、抽屉、按钮位置、表格列、操作入口、文案和交互流程为验收基准；Next 只翻新实现方式，不重新设计界面。
